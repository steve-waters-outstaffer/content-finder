...
class ContentPipeline:
    ...
    def run_full_pipeline(self, query: str, max_urls: int = 3, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        # Step 1: Search
        logger.info(f"PIPELINE: 1️⃣ SEARCHING for '{query}'")
        ...
        logger.info(f"PIPELINE: 📝 Found {len(urls)} URLs to process")
        pipeline_result['urls'] = urls

        # Step 2: Scrape
        logger.info(f"PIPELINE: 2️⃣ SCRAPING {len(urls)} URLs")
        ...
        logger.info(f"PIPELINE: ✓ Successfully scraped {len(successful_scrapes)}/{len(urls)} URLs")

        # Step 3: Extract structured data
        logger.info(f"PIPELINE: 3️⃣ EXTRACTING structured data from {len(urls)} URLs")
        extraction_result = self.firecrawl.extract_structured(urls)
        pipeline_result['steps']['extract'] = extraction_result

        if extraction_result.get('success'):
            logger.info("PIPELINE: ✓ Structured extraction completed")
        else:
            logger.warning(f"PIPELINE: ⚠️ Extraction failed: {extraction_result.get('error', 'Unknown error')}")

        # Step 4: Analyze with AI
        logger.info(f"PIPELINE: 4️⃣ ANALYZING content with Gemini")
        analyses = []
        for scrape_result in successful_scrapes:
            if scrape_result.get('markdown'):
                logger.info(f"PIPELINE: 🤖 Analyzing: {scrape_result['url']}")
        ...
        logger.info(f"PIPELINE: ✓ Analyzed {len(analyses)}/{len(successful_scrapes)} scraped documents")
